...
const changeUserStatus = async (user: IRequestUser, payload: IChangeUserStatusPayload) => {
    ...
    const { userId, userStatus } = payload; //Extracting data from the request body


    //Locating the user whose status is to be changed
    ...
    //You won't be able to change your status yourself.
    ...
    //Deleting a user is not just a status change.
    ...
    //It will update if all conditions are met.
